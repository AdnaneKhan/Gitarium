            ...
            TreeState::Ready => {
                ...
                dl.push_clip(inner);
                {
                    ...
                    for vis in 0..(inner.h / row_h) as usize + 2 {
                        ...
                        // Staged paths are tinted and carry a status dot.
                        let staged = rv.staged.get(&row.path);
                        let nc = match staged {
                            Some(Staged::Upsert(_)) => GREEN,
                            Some(Staged::Delete) => with_a(RED, 0.9),
                            None if row.is_dir => with_a(CYAN, 0.9),
                            None if selected => TEXT,
                            None => with_a(TEXT, 0.85),
                        };
                        let name = dl.fit(atlas, UI, self.f(14.0), &row.name, inner.right() - ix - self.f(28.0));
                        dl.text(atlas, UI, self.f(14.0), ix + self.f(16.0), baseline, &name, nc, 0.0);
                        if let Some(s) = staged {
                            let (m, c) = match s {
                                Staged::Upsert(_) => ("+", GREEN),
                                Staged::Delete => ("−", RED),
                            };
                            dl.text(atlas, MONO, self.f(12.0), inner.right() - self.f(14.0), baseline, m, c, 0.0);
                        }
