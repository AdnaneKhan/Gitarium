...
impl View {
    pub(super) fn editor_body(&mut self, app: &mut App, dl: &mut DrawList, atlas: &mut Atlas, body: RectF) {
        ...
        // Hyperlinks only while viewing: once editing, a plain click must place
        // the caret, so links go inert (no modifier reaches on_mouse_down).
        let links_on = !file.editing;
        ...
        for vis in 0..vis_rows {
            ...
            let links = if links_on {
                super::links::url_spans(line, &mut self.link_urls)
            } else {
                Vec::new()
            };
            // Selection band.
            ...
            for (ci, ch) in line.chars().enumerate() {
                ...
                let color = spans
                    ...
                    .unwrap_or(TEXT);
                // Link chars override the syntax color; the run loop flushes on
                // the color change, so each link becomes its own cyan run.
                let color = if links.iter().any(|&(a, b, _)| a <= ci && ci < b) { CYAN } else { color };
                if ch == '\t' {
            ...
            // Underline each link and register a click region (cursor-region
            // priority is handled in click_at). Cells account for tab expansion
            // and horizontal scroll, mirroring the run draw above.
            for &(a, b, u) in &links {
                let x0 = text_rect.x + ed.col_to_x(row, a) as f32 * adv - hscroll_px;
                let x1 = text_rect.x + ed.col_to_x(row, b) as f32 * adv - hscroll_px;
                dl.solid(RectF::new(x0, baseline + self.f(2.0), x1 - x0, self.f(1.0)), with_a(CYAN, 0.6));
                let hit = RectF::new(x0, y, x1 - x0, line_h).intersect(&body);
