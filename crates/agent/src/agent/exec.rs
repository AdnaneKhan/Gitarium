...
async fn exec_code_search(
    ...
    // In proxy mode the server may inject its own token, so an anonymous
    // browser session can still be authenticated server-side.
    if token.is_none() && !proxy::enabled() {
        return (
...
async fn exec_github(
    ...
    match proxy::github_request(method, &url, &headers, body.clone()).await {
        Ok(r) => {
